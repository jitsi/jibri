...
class StateTransitionTimeTracker(private val clock: Clock) {
    var timestampTransitionOccurred: Instant? = null
        private set

    fun maybeUpdate(eventOccurred: Boolean) {
        if (eventOccurred && timestampTransitionOccurred == null) {
            timestampTransitionOccurred = clock.instant()
        } else if (!eventOccurred) {
            timestampTransitionOccurred = null
        }
    }

    fun exceededTimeout(timeout: Duration): Boolean {
        return timestampTransitionOccurred?.let {
